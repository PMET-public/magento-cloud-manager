...
for proj in $(~/.magento-cloud/bin/magento-cloud projects --pipe); do
  deactivated_log=$(~/.magento-cloud/bin/magento-cloud activity:list -p $proj -e master -a --limit=500 2>/dev/null| grep 'deactivated environment' | awk '{print $4 " " $(NF-1)}')
  # reverse order with tac so inactive child envs deleted first 
  for inactive_env in $(~/.magento-cloud/bin/magento-cloud environments -p $proj 2>/dev/null | grep Inactive | awk '{print $2}' | tac); do
