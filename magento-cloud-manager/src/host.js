...
function updateHost(project, environment = 'master') {
  return exec(`${MC_CLI} ssh -p ${project} -e "${environment}" "
    ...
    .then(({ stdout, stderr }) => {
      ...
      return db.prepare(`INSERT INTO hosts_states (project_id, environment_id, boot_time, ip, total_memory, cpus, load_avg_1, 
...
async function updateHostsUsingSampleProjects() {
  ...
  const groupedProjects = db.prepare(`SELECT GROUP_CONCAT(DISTINCT p.id) projects FROM hosts_states h LEFT JOIN 
    projects p ON h.project_id = p.id GROUP BY h.boot_time, h.cpus, h.ip`).all();
