...
CREATE TABLE IF NOT EXISTS "web_statuses_new" (
  ...
  -- expiration will update periodically but not meaningfully
  -- only interested when expiration status changes i.e. not expired -> expired
  `last_meaningful_change_ts` INTEGER NOT NULL DEFAULT (cast(strftime('%s','now') as int)),
  `timestamp` INTEGER NOT NULL DEFAULT (cast(strftime('%s','now') as int)),
  PRIMARY KEY(`host_name`)
);
CREATE TABLE IF NOT EXISTS "status_reports" (
  `envs_changed` INTEGER NOT NULL,
  `timestamp` INTEGER NOT NULL DEFAULT (cast(strftime('%s','now') as int))
);
